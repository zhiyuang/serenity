...
void AntiAliasingPainter::stroke_path(Path const& path, Color color, float thickness, Path::StrokeLinecap liencap)
{
    if (thickness <= 0)
        return;
    // FIXME: Cache this? Probably at a higher level such as in LibWeb?
    fill_path(path.stroke_to_fill(thickness, liencap), color);
}

void AntiAliasingPainter::stroke_path(Path const& path, Gfx::PaintStyle const& paint_style, float opacity, float thickness, Path::StrokeLinecap liencap)
{
    if (thickness <= 0)
        return;
    // FIXME: Cache this? Probably at a higher level such as in LibWeb?
    fill_path(path.stroke_to_fill(thickness, liencap), paint_style, opacity);
}
